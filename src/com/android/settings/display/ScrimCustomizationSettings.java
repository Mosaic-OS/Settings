...
public class ScrimCustomizationSettings extends DashboardFragment {
    ...
    private void resetToDefaults() {
        // Set the value on each preference; the custom Seekbar/Switch preferences persist
        // to Settings.Secure themselves (and update the UI). Writing the keys directly here
        // as well would persist every value twice, so rely solely on the preferences.
        CustomSeekBarPreference bgBlur = findPreference("background_blur_radius");
        if (bgBlur != null) bgBlur.setProgress(DEFAULT_BACKGROUND_BLUR);

        CustomSeekBarPreference bgTransparency = findPreference("background_transparency");
        if (bgTransparency != null) bgTransparency.setProgress(DEFAULT_BACKGROUND_TRANSPARENCY);

        CustomSeekBarPreference notifTransparency = findPreference("notification_transparency");
        if (notifTransparency != null) notifTransparency.setProgress(DEFAULT_NOTIFICATION_TRANSPARENCY);

        CustomSwitchPreference bgAccent = findPreference("background_accent");
        if (bgAccent != null) bgAccent.setChecked(DEFAULT_BACKGROUND_ACCENT);

        CustomSwitchPreference notifAccent = findPreference("notification_accent");
        if (notifAccent != null) notifAccent.setChecked(DEFAULT_NOTIFICATION_ACCENT);
    }
