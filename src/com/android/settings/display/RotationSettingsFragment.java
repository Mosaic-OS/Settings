...
public class RotationSettingsFragment extends SettingsPreferenceFragment {
    ...
    // Keep the angle checkboxes in sync when ACCELEROMETER_ROTATION_ANGLES is changed
    // (e.g. externally); the RotationPolicyListener above only fires on rotation-policy
    // changes, not on edits to the angle bitmask itself.
    ...
    public void onCreatePreferences(@Nullable Bundle savedInstanceState,
        ...
        // This fragment is also reachable via the exported AUTO_ROTATE_SETTINGS activity,
        // which performs no sensor check. Gate the whole screen on rotation support so we
        // never show or write rotation settings on a device without the sensor.
