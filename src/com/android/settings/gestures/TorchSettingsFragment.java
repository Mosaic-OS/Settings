...
/** Torch settings: main switch, auto-off timeout and vibration feedback. */
...
public class TorchSettingsFragment extends SettingsPreferenceFragment
    ...
    private static final String KEY_VIBRATE = "torch_long_press_power_vibrate";
    ...
    private SwitchPreferenceCompat mVibratePref;
    ...
    public void onCreatePreferences(Bundle savedInstanceState, String rootKey) {
        ...
        mVibratePref = findPreference(KEY_VIBRATE);
    ...
    public void onResume() {
        ...
        mResolver.registerContentObserver(Settings.Secure.getUriFor(KEY_TIMEOUT),
                false, mSettingsObserver, UserHandle.USER_ALL);
        mResolver.registerContentObserver(Settings.Secure.getUriFor(KEY_VIBRATE),
                false, mSettingsObserver, UserHandle.USER_ALL);
        updateUiFromSettings();
    }
    ...
    private void updateUiFromSettings() {
        ...
        if (mTimeoutPref != null) {
            ...
            if (idx >= 0) {
                ...
                // Off-list but valid (adb/restore): show the real value, not a false "Never".
                mTimeoutPref.setSummary(getString(
                        R.string.torch_long_press_power_timeout_custom, sec));
            }
        ...
        if (mVibratePref != null) {
            int v = Settings.Secure.getInt(mResolver, KEY_VIBRATE, DEFAULT_VIBRATE);
            mVibratePref.setChecked(v != 0);
            if (mMainSwitch != null) {
                mVibratePref.setEnabled(mMainSwitch.isChecked());
            }
        }
    }
    ...
    public boolean onPreferenceChange(Preference preference, Object newValue) {
        if (preference == mTimeoutPref) {
            ...
            boolean ok = Settings.Secure.putInt(mResolver, KEY_VIBRATE, enabled ? 1 : 0);
            if (!ok) {
                Slog.w(TAG, "Failed to write " + KEY_VIBRATE + " = " + enabled);
                return false;
            }
            return true;
        }
    ...
    public static final BaseSearchIndexProvider SEARCH_INDEX_DATA_PROVIDER =
            new BaseSearchIndexProvider(R.xml.gesture_torch_settings);
}
